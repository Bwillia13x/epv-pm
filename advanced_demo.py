...
def demo_advanced_valuations():
    ...
    try:
        ...
        symbol = "DEMO"
        income_statements = [
            IncomeStatement(symbol, "annual", 2023, net_income=1000000, revenue=10000000, eps=10.0, shares_outstanding=100000),
            IncomeStatement(symbol, "annual", 2022, net_income=900000, revenue=9500000, eps=9.0, shares_outstanding=100000),
            IncomeStatement(symbol, "annual", 2021, net_income=800000, revenue=9000000, eps=8.0, shares_outstanding=100000),
        ]
        
        balance_sheets = [
            BalanceSheet(symbol, "annual", 2023, total_assets=5000000, total_equity=3000000, long_term_debt=1000000,
                        current_assets=2000000, current_liabilities=1000000, cash_and_equivalents=500000),
            BalanceSheet(symbol, "annual", 2022, total_assets=4500000, total_equity=2700000, long_term_debt=900000,
                        current_assets=1800000, current_liabilities=900000, cash_and_equivalents=400000),
        ]
        
        cash_flows = [
            CashFlowStatement(symbol, "annual", 2023, operating_cash_flow=1200000, free_cash_flow=800000, capital_expenditures=-400000),
            CashFlowStatement(symbol, "annual", 2022, operating_cash_flow=1100000, free_cash_flow=700000, capital_expenditures=-400000),
        ]
        ...
        print(f"  • Tangible book value per share: ${asset_result.tangible_book_value_per_share:.2f}")
        print(f"  • Liquidation value per share: ${asset_result.liquidation_value_per_share:.2f}")
        ...
        print(f"  • Average multiples value: ${multiples_result.multiples_average_value:.2f}")
...
def demo_portfolio_management():
    ...
    try:
        ...
        weighted_epv = sum(pos.epv_per_share * (pos.market_value / total_value) for pos in positions)
        weighted_mos = sum(pos.epv_margin_of_safety * (pos.market_value / total_value) for pos in positions)
        ...
        if rebalancing:
            print(f"  • Rebalancing needed: Max deviation {rebalancing.current_deviation:.1%}")
...
def demo_alternative_data():
    ...
    try:
        ...
        sec_filings = alt_data['data_sources'].get('sec_filings', {}).get('sec_filings', [])
        if sec_filings:
            latest_filing = sec_filings[0]
            print(f"  • Latest filing: {latest_filing.filing_type} ({latest_filing.filing_date})")
        ...
        insider_data = alt_data['data_sources'].get('insider_trading', {}).get('insider_trading', [])
        if insider_data:
            print(f"  • Recent transactions: {len(insider_data)}")
            for transaction in insider_data[:2]:  # Show first 2
                print(f"    - {transaction.insider_name} ({transaction.title}): "
        ...
        if esg_data:
            print(f"  • Overall ESG score: {esg_data.overall_esg_score:.1f}/100 (Rating: {esg_data.esg_rating})")
        ...
        print("\n📰 Sentiment Analysis:")
        sentiment_data = alt_data['data_sources'].get('sentiment_data', {}).get('sentiment_analysis')
        if sentiment_data:
            print(f"  • Overall sentiment: {sentiment_data.overall_sentiment:.2f} (-1 to +1)")
        ...
        mgmt_data = alt_data['data_sources'].get('management_data', {}).get('management_assessment')
        if mgmt_data:
            print(f"  • CEO: {mgmt_data.ceo_name} (Tenure: {mgmt_data.ceo_tenure_years:.1f} years)")
            print(f"  • Historical performance score: {mgmt_data.historical_performance_score:.2f}")
            print(f"  • Capital allocation score: {mgmt_data.capital_allocation_score:.2f}")
            print(f"  • Strategic execution score: {mgmt_data.strategic_execution_score:.2f}")
        ...
        print(f"  • Overall Alternative Data Score: {scores['overall_alternative_score']:.2f}")
...
def demo_professional_reporting():
    ...
    try:
        ...
        try:
            ...
            full_pdf = report_generator.generate_comprehensive_pdf_report(research_report)
