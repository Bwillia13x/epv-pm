...
def upgrade() -> None:
    ...
    op.create_table(
        "financial_statement",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("company_id", sa.Integer(), sa.ForeignKey("company_profile.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index(
        "ix_market_data_company_date", "market_data", ["company_id", "date"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.Column("report_type", sa.String(length=50), nullable=False, server_default="executive_summary"),
        sa.Column("data", sa.JSON(), nullable=False),
