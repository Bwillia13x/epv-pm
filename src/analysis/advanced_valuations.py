...
class AdvancedValuationEngine:
    ...
    def calculate_dcf_valuation(
        ...
        try:
            ...
            if discount_rate is None:
                discount_rate = self._estimate_discount_rate(
                    balance_sheets, income_statements
                )

            # Project revenues
            revenue_projections = self._project_revenues(
                income_statements, projection_years
            )
    ...
    def calculate_asset_based_valuation(
        ...
        try:
            ...
            book_value_per_share = (
                latest_bs.total_equity / shares_outstanding
            ...
            if latest_bs.total_assets and latest_bs.total_equity:
                ...
                intangible_assets = max(
                    0, latest_bs.total_assets * 0.1
                )  # Conservative estimate

            tangible_book_value = latest_bs.total_equity - intangible_assets
            tangible_book_value_per_share = (
                tangible_book_value / shares_outstanding
            ...
            adjusted_book_value = (
                latest_bs.total_equity
                + total_asset_adjustments
                - total_liability_adjustments
            )
            adjusted_book_value_per_share = (
                adjusted_book_value / shares_outstanding
                if shares_outstanding > 0
                else 0
            )
            ...
            liquidation_value = (
                adjusted_book_value * 0.7
            )  # Assume 30% discount in liquidation
            liquidation_value_per_share = (
                liquidation_value / shares_outstanding if shares_outstanding > 0 else 0
            )
            ...
            self.logger.error(
                f"Error calculating asset-based valuation for {symbol}: {e}"
            )
            raise

    def calculate_market_multiples_valuation(
        self,
        symbol: str,
        income_statements: List[IncomeStatement],
        balance_sheets: List[BalanceSheet],
        ...
        try:
            ...
            market_cap = (
                ...
                if current_price and shares_outstanding > 0
                else None
            )
            ...
            if latest_bs.total_equity and shares_outstanding > 0:
                book_value_per_share = latest_bs.total_equity / shares_outstanding
                pb_based_value = book_value_per_share * industry_multiples.get(
                    "pb", 2.0
                )
            ...
            self.logger.error(
                f"Error calculating market multiples valuation for {symbol}: {e}"
            )
            raise
    ...
    def run_monte_carlo_simulation(
        ...
        try:
            ...
            probability_of_loss = np.mean(
                simulation_results < base_valuation * 0.8
            )  # 20% loss threshold
            upside_potential = np.mean(
                simulation_results > base_valuation * 1.2
            )  # 20% gain threshold
    ...
    def _project_revenues(
        self, income_statements: List[IncomeStatement], years: int
    ) -> List[float]:
        ...
        for year in range(years):
            ...
            growth_rate = recent_growth * (0.8**year) + long_term_growth * (
                1 - 0.8**year
            )
            ...
            if cf_stmt.free_cash_flow:
                ...
                matching_income = next(
                    (
                        stmt
                        for stmt in income_statements
                        if stmt.fiscal_year == cf_stmt.fiscal_year
                    ),
                    None,
                )
                if (
                    matching_income
                    and matching_income.revenue
                    and matching_income.revenue > 0
                ):
    ...
    def _dcf_sensitivity_analysis(
        ...
        for dr in discount_rates:
            pv_fcf = sum(
                fcf / (1 + dr) ** (i + 1) for i, fcf in enumerate(fcf_projections)
            )
            terminal_value = (
                terminal_fcf
                / (dr - base_terminal_growth)
                / (1 + dr) ** len(fcf_projections)
            )
        ...
        for gr in growth_rates:
            if gr < base_discount_rate:  # Ensure growth < discount rate
                pv_fcf = sum(
                    fcf / (1 + base_discount_rate) ** (i + 1)
                    for i, fcf in enumerate(fcf_projections)
                )
                terminal_value = (
                    terminal_fcf
                    / (base_discount_rate - gr)
                    / (1 + base_discount_rate) ** len(fcf_projections)
                )
                equity_value = pv_fcf + terminal_value + net_cash
                per_share_value = (
                    equity_value / shares_outstanding if shares_outstanding > 0 else 0
    ...
    def _calculate_asset_adjustments(
    ...
    def _calculate_liability_adjustments(
        self, balance_sheet: BalanceSheet
    ) -> Dict[str, float]:
        ...
        if balance_sheet.total_assets:
            adjustments["off_balance_sheet_estimate"] = (
                balance_sheet.total_assets * 0.02
            )
