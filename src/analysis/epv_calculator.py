...
class EPVCalculator:
    ...
    _cache: Dict[str, EPVCalculation] = {}
    ...
    def calculate_epv(
        ...
        # Memoization – only compute once per symbol per process
        ...
        try:
            ...
            self.logger.info(f"Normalized earnings: ${normalized_earnings:,.0f}")

            # Step 2: Get current shares outstanding
            shares_outstanding = self._get_current_shares_outstanding(income_statements)
            self.logger.info(f"Shares outstanding: {shares_outstanding:,.0f}")

            # Step 3: Calculate cost of capital (WACC approximation)
            cost_of_capital = self._calculate_cost_of_capital(
                balance_sheets, financial_ratios, company_profile
            )
            self.logger.info(f"Cost of capital: {cost_of_capital:.2%}")

            # Step 4: Calculate EPV components
            earnings_per_share = normalized_earnings / shares_outstanding
            epv_per_share = earnings_per_share / cost_of_capital
            epv_total = normalized_earnings / cost_of_capital

            # Step 5: Calculate margin of safety
            margin_of_safety = None
            if current_price:
                margin_of_safety = (
                    (epv_per_share - current_price) / current_price
                ) * 100
            ...
            self.logger.info(
                f"EPV calculation complete for {symbol}: ${epv_per_share:.2f} per share"
            )

            # store in cache thread-safely
            with self._lock:
                self._cache[symbol] = epv_calculation

            return epv_calculation

        except Exception as e:
            self.logger.error(f"Error calculating EPV for {symbol}: {e}")
            raise

    def _calculate_normalized_earnings(
        ...
        operating_incomes = [
            e["operating_income"] for e in earnings_data if e["operating_income"]
        ]
        ...
        if avg_operating_income > 0:
            ...
            normalized_earnings = (0.6 * avg_operating_income * 0.7) + (
                0.4 * weighted_avg
            )
        else:
    ...
    def _get_current_shares_outstanding(
        self, income_statements: List[IncomeStatement]
    ) -> float:
        """Get current shares outstanding from most recent data"""

        for stmt in sorted(
            income_statements, key=lambda x: x.fiscal_year, reverse=True
        ):
    ...
    def _calculate_cost_of_capital(
        self,
        balance_sheets: List[BalanceSheet],
        financial_ratios: List[FinancialRatios],
        ...
        if financial_ratios:
            quality_metrics = self._calculate_quality_metrics(
                financial_ratios, balance_sheets
            )
    ...
    def _assess_quality(
        ...
        if income_statements:
            earnings = [
                stmt.net_income for stmt in income_statements if stmt.net_income
            ]
            if len(earnings) >= 3:
                earnings_cv = (
                    np.std(earnings) / np.mean(earnings) if np.mean(earnings) > 0 else 1
                )
        ...
        if financial_ratios:
            roes = [
                ratio.roe for ratio in financial_ratios if ratio.roe and ratio.roe > 0
            ]
            if roes:
        ...
        if balance_sheets:
            ...
            if latest_bs.long_term_debt and latest_bs.total_equity:
                ...
                quality_components["leverage_quality"] = max(
                    0, 1 - min(1, debt_to_equity / 2)
                )
            else:
                quality_components["leverage_quality"] = (
                    0.8  # Assume low debt if no data
                )
            ...
            if latest_bs.current_assets and latest_bs.current_liabilities:
                ...
                if 1.5 <= current_ratio <= 3.0:
                    ...
                    quality_components["liquidity_quality"] = (
                        0.8  # Too much cash might be inefficient
                    )
                else:
                    quality_components["liquidity_quality"] = max(
                        0, current_ratio / 1.5
                    )
            else:
            ...
            if len(revenues) >= 3:
                ...
                if growth_rates:
                    ...
                    if avg_growth >= 0 and growth_volatility < 0.2:
                        quality_components["growth_quality"] = min(
                            1.0, 0.5 + avg_growth * 2
                        )
                    else:
                        quality_components["growth_quality"] = max(
                            0, 0.5 - growth_volatility
                        )
                else:
        ...
        if financial_ratios:
            ...
            if roes:
                ...
                metrics["roe_stability"] = (
                    1 - (np.std(roes) / np.mean(roes)) if np.mean(roes) > 0 else 0
                )
    ...
    def _calculate_growth_scenarios(
        self,
        ...
        for growth_rate in growth_rates:
            if growth_rate < cost_of_capital:  # Ensure growth < discount rate
                ...
                value_per_share = (
                    (normalized_earnings * (1 + growth_rate))
                    / (cost_of_capital - growth_rate)
                    / shares_outstanding
                )
    ...
    def _quality_recommendations(self, components: Dict[str, float]) -> List[str]:
        ...
        if components.get("earnings_stability", 0) < 0.5:
            recommendations.append(
                "Monitor earnings volatility - consider if cyclical factors explain variation"
            )

        if components.get("roe_quality", 0) < 0.5:
            recommendations.append(
                "Low return on equity - investigate capital allocation efficiency"
            )

        if components.get("leverage_quality", 0) < 0.5:
            recommendations.append(
                "High leverage - assess debt sustainability and refinancing risks"
            )

        if components.get("liquidity_quality", 0) < 0.5:
            recommendations.append(
                "Liquidity concerns - monitor working capital and cash flow"
            )

        if components.get("growth_quality", 0) < 0.5:
            recommendations.append(
                "Volatile revenue growth - understand market dynamics and competitive position"
            )

        if not recommendations:
            recommendations.append(
                "Strong financial profile across key quality metrics"
            )
