...
class ResearchGenerator:
    ...
    def generate_research_report(
        ...
        try:
            ...
            company_data = self.data_collector.collect_company_data(
                symbol, years_lookback
            )
            ...
            quality_score, quality_analysis = (
                self.epv_calculator.calculate_quality_score(
            ...
            recommendation, target_price = self._generate_recommendation(
                epv_calculation, quality_score, risk_score
            )
            ...
            report = ResearchReport(
                ...
                cash_flow_statements=company_data["cash_flow_statements"],
                financial_ratios=company_data["financial_ratios"],
                current_market_data=self._create_current_market_data(
                    symbol, current_price
                ),
                historical_prices=(
                    company_data["market_data"][-252:]
            ...
            self.logger.info(
                f"Research report generated for {symbol}: {recommendation}"
            )
            return report
    ...
    def _create_current_market_data(
        self, symbol: str, price: Optional[float]
    ) -> Optional[MarketData]:
        """Create current market data snapshot"""
        if price:
            return MarketData(symbol=symbol, date=date.today(), price=price)
        return None
    ...
    def _generate_peer_comparisons(self, symbol: str, peer_symbols: List[str]) -> Dict:
        ...
        try:
            ...
            peer_data = self.data_collector.get_peer_comparison_data(
                symbol, peer_symbols
            )
            ...
            if peer_metrics:
                ...
                comparisons["summary"] = self._summarize_peer_comparison(
                    symbol, peer_metrics
                )
    ...
    def _assess_risks(
        self, company_data: Dict, epv_calculation: EPVCalculation
        ...
        try:
            ...
            if income_statements and len(income_statements) >= 3:
                ...
                if earnings:
                    earnings_cv = (
                        np.std(earnings) / np.mean(earnings)
                        if np.mean(earnings) > 0
                        else 1
                    )
                    if earnings_cv > 0.5:
                        risk_factors.append(
                            "High earnings volatility - unpredictable income stream"
                        )
                        risk_score += 0.15
            ...
            if balance_sheets:
                ...
                if latest_bs.current_assets and latest_bs.current_liabilities:
                    current_ratio = (
                        latest_bs.current_assets / latest_bs.current_liabilities
                    )
                    if current_ratio < 1.0:
                        risk_factors.append(
                            "Liquidity concerns - current ratio below 1.0"
                        )
                        risk_score += 0.15
            ...
            if epv_calculation.quality_score is not None:
                if epv_calculation.quality_score < 0.3:
                    risk_factors.append(
                        "Low quality business - earnings sustainability concerns"
                    )
                    risk_score += 0.2
            ...
            if epv_calculation.margin_of_safety is not None:
                if (
                    epv_calculation.margin_of_safety < -50
                ):  # Trading more than 50% above EPV
                    risk_factors.append(
                        "Significant overvaluation risk - trading well above intrinsic value"
                    )
                    risk_score += 0.15
                elif epv_calculation.margin_of_safety < -20:
                    risk_factors.append(
                        "Moderate overvaluation - limited margin of safety"
                    )
                    risk_score += 0.05
            ...
            if income_statements and len(income_statements) >= 3:
                ...
                if len(revenues) >= 3:
                    recent_growth = (
                        (revenues[-1][1] - revenues[-3][1]) / revenues[-3][1] / 2
                    )  # 2-year CAGR
                    if recent_growth < -0.05:  # Declining revenue
                        risk_factors.append(
                            "Revenue decline trend - market share or demand concerns"
                        )
                        risk_score += 0.15
    ...
    def _generate_investment_thesis(
        ...
        # Company overview
        thesis_parts.append(
            f"{profile.company_name} ({epv_calculation.symbol}) operates in the {profile.sector or 'N/A'} sector"
        )
        ...
        if epv_calculation.current_price:
            ...
            if epv_calculation.margin_of_safety is not None:
                if epv_calculation.margin_of_safety > 20:
                    thesis_parts.append(
                        f"Trading at significant discount to EPV ({epv_calculation.margin_of_safety:.1f}% margin of safety)"
                    )
                elif epv_calculation.margin_of_safety > 0:
                    thesis_parts.append(
                        f"Trading below EPV with {epv_calculation.margin_of_safety:.1f}% margin of safety"
                    )
                else:
                    thesis_parts.append(
                        f"Trading above EPV by {abs(epv_calculation.margin_of_safety):.1f}%"
                    )
        ...
        if recommendations:
            positive_recs = [
                ...
                if "strong" in rec.lower() or "good" in rec.lower()
            ]
            concern_recs = [
                rec
                for rec in recommendations
                if any(
                    word in rec.lower()
                    for word in ["monitor", "concern", "low", "high leverage"]
                )
            ]
            ...
            if summary:
                ...
                if avg_peer_epv and epv_calculation.epv_per_share:
                    if epv_calculation.epv_per_share > avg_peer_epv * 1.1:
                        thesis_parts.append(
                            f"EPV exceeds peer average by {((epv_calculation.epv_per_share / avg_peer_epv - 1) * 100):.1f}%"
                        )
                    elif epv_calculation.epv_per_share < avg_peer_epv * 0.9:
                        thesis_parts.append(
                            f"EPV below peer average by {((1 - epv_calculation.epv_per_share / avg_peer_epv) * 100):.1f}%"
                        )

        return ". ".join(thesis_parts) + "."

    def _generate_recommendation(
        self, epv_calculation: EPVCalculation, quality_score: float, risk_score: float
    ...
    def _calculate_confidence_level(
        self, quality_score: float, risk_score: float
    ) -> float:
    ...
    def _export_csv(self, report: ResearchReport) -> str:
        ...
        data = {
            ...
            "EPV_Per_Share": [
                report.epv_calculation.epv_per_share if report.epv_calculation else None
            ],
            "Current_Price": [
                report.epv_calculation.current_price if report.epv_calculation else None
            ],
            "Margin_of_Safety": [
                (
