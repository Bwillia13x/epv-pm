...
from models.financial_models import EPVCalculation, MarketData, PortfolioPosition
...
class PortfolioManager:
    ...
    def optimize_portfolio(self, 
                         ...
                         optimization_objective: str = 'max_epv_quality') -> List[PortfolioAllocation]:
        """
        ...
        try:
            ...
            for i, (symbol, weight) in enumerate(zip(symbols, weights)):
                if weight > 0.01:  # Only include meaningful allocations
                    ...
                    shares_to_trade = dollar_amount / current_prices[i] if current_prices[i] > 0 else 0
                    ...
                    allocation = PortfolioAllocation(
                        ...
                        margin_of_safety=((epv_values[i] - current_prices[i]) / current_prices[i] * 100 
                                        if current_prices[i] > 0 else 0),
                        quality_score=quality_scores[i],
                        conviction_level=weight * quality_scores[i]  # Weight by quality
                    )
            ...
            self.logger.info(f"Portfolio optimization complete: {len(allocations)} positions recommended")
            return allocations
    ...
    def calculate_portfolio_metrics(self,
                                  ...
                                  benchmark_returns: Optional[List[float]] = None) -> PortfolioMetrics:
        """Calculate comprehensive portfolio metrics"""
        
        self.logger.info(f"Calculating metrics for portfolio with {len(positions)} positions")
        
        try:
            # Calculate portfolio value and weights
            total_value = sum(pos.market_value for pos in positions)
            weights = [pos.market_value / total_value for pos in positions] if total_value > 0 else []
            
            # Calculate returns
            portfolio_returns = self._calculate_portfolio_returns(positions, historical_prices)
            
            if len(portfolio_returns) == 0:
                raise ValueError("No historical data available for portfolio metrics")
            
            # Performance metrics
            total_return = (portfolio_returns[-1] - portfolio_returns[0]) / portfolio_returns[0] if len(portfolio_returns) > 1 else 0
            ...
            # Risk metrics
            portfolio_beta = self._calculate_portfolio_beta(daily_returns, benchmark_returns)
            tracking_error = self._calculate_tracking_error(daily_returns, benchmark_returns)
            
            # Value at Risk (5%)
            var_5pct = np.percentile(daily_returns, 5) * np.sqrt(252) if len(daily_returns) > 0 else 0
            
            # Expected Shortfall (Conditional VaR)
            expected_shortfall = np.mean(daily_returns[daily_returns <= np.percentile(daily_returns, 5)]) * np.sqrt(252) if len(daily_returns) > 0 else 0
            
            # EPV-specific metrics
            weighted_epv = sum(pos.epv_per_share * weight for pos, weight in zip(positions, weights))
            weighted_margin_of_safety = sum(pos.epv_margin_of_safety * weight for pos, weight in zip(positions, weights))
            
            # Quality score (would need to be passed in or calculated)
            weighted_quality_score = 0.7  # Placeholder
            
            # EPV to market ratio
            weighted_market_price = sum(pos.current_price * weight for pos, weight in zip(positions, weights))
            epv_to_market_ratio = weighted_epv / weighted_market_price if weighted_market_price > 0 else 0
    ...
    def generate_rebalancing_recommendation(self,
                                          ...
                                          transaction_cost: float = 0.001) -> Optional[RebalancingRecommendation]:
        ...
        try:
            ...
            current_allocations = {
                ...
                for pos in current_positions if total_value > 0
            }
            
            # Calculate target allocations
            target_allocations_dict = {
                alloc.symbol: alloc.target_weight 
                for alloc in target_allocations
            }
            
            # Calculate deviations
            deviations = {}
            for symbol in set(list(current_allocations.keys()) + list(target_allocations_dict.keys())):
            ...
            if max_deviation < rebalancing_threshold:
                self.logger.info(f"No rebalancing needed (max deviation: {max_deviation:.1%})")
                return None
            ...
            for alloc in target_allocations:
                ...
                if abs(weight_diff) > rebalancing_threshold:
                    ...
                    trade = PortfolioAllocation(
                        ...
                        shares_to_trade=dollar_amount / alloc.current_price if alloc.current_price > 0 else 0,
    ...
    def create_risk_budget(self,
                          ...
                          max_sector_exposure: float = 0.3) -> RiskBudget:
    ...
    def _optimize_epv_quality(self, 
                            expected_returns: np.ndarray,
                            quality_scores: np.ndarray,
                            cov_matrix: np.ndarray,
                            risk_budget: RiskBudget) -> np.ndarray:
        ...
        def objective(weights):
            ...
            return -(portfolio_return * portfolio_quality - risk_penalty * portfolio_risk)
    ...
    def _calculate_portfolio_returns(self,
                                   positions: List[PortfolioPosition],
                                   historical_prices: Dict[str, List[MarketData]]) -> List[float]:
        ...
        for date_point in common_dates:
            ...
            for position in positions:
                ...
                if symbol in historical_prices:
                    ...
                    day_price = next(
                        (p.price for p in historical_prices[symbol] if p.date == date_point),
                        None
                    )
    ...
    def _calculate_tracking_error(self,
                                portfolio_returns: np.ndarray,
                                benchmark_returns: Optional[List[float]]) -> Optional[float]:
    ...
    def _estimate_rebalancing_benefit(self,
                                    current_allocations: Dict[str, float],
                                    target_allocations: Dict[str, float]) -> float:
