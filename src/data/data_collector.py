...
class YahooFinanceSource(DataSource):
    ...
    def _calculate_free_cash_flow(self, cash_flow: pd.DataFrame, column) -> Optional[float]:
        """Calculate free cash flow"""
        try:
            ocf = self._safe_get(cash_flow, 'Total Cash From Operating Activities', column)
    ...
    def _calculate_ratios(self, data: Dict) -> List[FinancialRatios]:
        ...
        try:
            ...
            for income_stmt in income_stmts:
                matching_bs = next(
                    (bs for bs in balance_sheets if bs.fiscal_year == income_stmt.fiscal_year), 
                    None
                )
                ...
                if matching_bs and income_stmt.net_income and matching_bs.total_equity:
                    ...
                    if income_stmt.net_income and matching_bs.total_equity:
                        ratio.roe = (income_stmt.net_income / matching_bs.total_equity) * 100
                    
                    if income_stmt.net_income and matching_bs.total_assets:
                        ratio.roa = (income_stmt.net_income / matching_bs.total_assets) * 100
                    
                    if income_stmt.revenue and income_stmt.gross_profit:
                        ratio.gross_margin = (income_stmt.gross_profit / income_stmt.revenue) * 100
                    
                    if income_stmt.revenue and income_stmt.operating_income:
                        ratio.operating_margin = (income_stmt.operating_income / income_stmt.revenue) * 100
                    
                    if income_stmt.revenue and income_stmt.net_income:
                        ratio.net_margin = (income_stmt.net_income / income_stmt.revenue) * 100
                    
                    # Calculate liquidity ratios
                    if matching_bs.current_assets and matching_bs.current_liabilities:
                        ratio.current_ratio = matching_bs.current_assets / matching_bs.current_liabilities
                    
                    # Calculate leverage ratios
                    if matching_bs.long_term_debt and matching_bs.total_equity:
                        ratio.debt_to_equity = matching_bs.long_term_debt / matching_bs.total_equity
