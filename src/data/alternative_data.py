...
class InsiderTradingProvider(AlternativeDataProvider):
    ...
    def collect_data(self, symbol: str) -> Dict[str, List[InsiderTrading]]:
        ...
        try:
            ...
            self.logger.error(f"Error collecting insider trading data for {symbol}: {e}")
            return {'insider_trading': []}
    
    def _get_insider_transactions(self, symbol: str, days_back: int = 90) -> List[InsiderTrading]:
        ...
        for i in range(np.random.randint(0, 5)):  # 0-4 transactions
            transaction_date = date.today() - timedelta(days=np.random.randint(1, days_back))
...
class ManagementDataProvider(AlternativeDataProvider):
    ...
    def _assess_management(self, symbol: str) -> ManagementAssessment:
        ...
        return ManagementAssessment(
            ...
            governance_issues=[] if capital_allocation > 0.6 else ["Executive compensation concerns"],
            compensation_concerns=[] if historical_performance > 0.6 else ["Pay-for-performance alignment"],
...
class AlternativeDataCollector:
    ...
    def _calculate_intelligence_scores(self, data_sources: Dict) -> Dict[str, float]:
        ...
        try:
            ...
            if esg_data:
                scores['esg_score'] = esg_data.overall_esg_score / 100 if esg_data.overall_esg_score else 0.5
            else:
                scores['esg_score'] = 0.5
            
            # Sentiment score (normalized to 0-1)
            sentiment_data = data_sources.get('sentiment_data', {}).get('sentiment_analysis')
            if sentiment_data and sentiment_data.overall_sentiment is not None:
                scores['sentiment_score'] = (sentiment_data.overall_sentiment + 1) / 2  # Convert from -1,1 to 0,1
            else:
                scores['sentiment_score'] = 0.5
            
            # Management quality score
            mgmt_data = data_sources.get('management_data', {}).get('management_assessment')
            if mgmt_data:
                mgmt_scores = [
            ...
            insider_data = data_sources.get('insider_trading', {}).get('insider_trading', [])
            if insider_data:
                ...
                scores['insider_trading_score'] = buy_signal - sell_signal * 0.5 + 0.5  # Normalize
            else:
            ...
            if sec_data:
                sentiment_scores = [f.sentiment_score for f in sec_data if f.sentiment_score is not None]
                if sentiment_scores:
                    avg_sentiment = np.mean(sentiment_scores)
                    scores['filing_quality_score'] = (avg_sentiment + 1) / 2  # Normalize
                else:
            ...
            scores['overall_alternative_score'] = np.mean(score_values) if score_values else 0.5
    ...
    def get_red_flags(self, symbol: str, data_sources: Dict) -> List[str]:
        ...
        try:
            ...
            insider_data = data_sources.get('insider_trading', {}).get('insider_trading', [])
            heavy_selling = [t for t in insider_data if t.transaction_type == 'Sell' and t.significance_score > 0.7]
            if len(heavy_selling) > 2:
                red_flags.append("Multiple significant insider sales")
            
            # Sentiment red flags
            sentiment_data = data_sources.get('sentiment_data', {}).get('sentiment_analysis')
            if sentiment_data and sentiment_data.overall_sentiment and sentiment_data.overall_sentiment < -0.3:
