    ...
    def http_client(self):
        ...
        try:
            with httpx.Client(base_url=self.BASE_URL, timeout=5.0) as client:
                # Quick health check to see if the backend is running.
                try:
                    health_resp = client.get("/health")
                    if health_resp.status_code != 200:
                        pytest.skip(
                            "Backend API not running on localhost:8000 – skipping integration tests"
                        )
                except (httpx.TransportError, httpx.HTTPError):
                    pytest.skip(
                        "Backend API not reachable at localhost:8000 – skipping integration tests"
                    )

                yield client
        except Exception as e:
            pytest.skip(f"Failed to create HTTP client: {e}")

    def test_01_user_registration(
        self, http_client: httpx.Client, test_credentials: Dict[str, str]
    ):
        ...
        if response.status_code == 404:
            pytest.skip(
                "Registration endpoint not available - auth system may not be fully configured"
            )
    ...
    def test_02_user_login(
        ...
        login_data = {
            "username": test_credentials[
                "email"
            ],  # fastapi-users uses 'username' field
            "password": test_credentials["password"],
        }
        ...
        if response.status_code == 404:
            pytest.skip(
                "Login endpoint not available - auth system may not be fully configured"
            )
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if "/dashboard" not in page.url:
                    dashboard_link = (
                        page.locator("a").filter(has_text="Dashboard").first
                    )
                ...
                pdf_button = (
                    page.locator("button")
                    .filter(has_text=["PDF", "Generate", "Download"])
                    .first
                )
                if pdf_button.count() > 0:
                    ...
                    assert (
                        file_size > 10240
                    ), f"PDF file too small: {file_size} bytes"  # >10KB
                ...
                page.screenshot(
                    path=f"/tmp/integration_test_failure_{int(time.time())}.png"
                )
                raise
            finally:
                browser.close()
