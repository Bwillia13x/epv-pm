...
def mock_yahoo_source(monkeypatch):
    ...
    mock.get_quote = AsyncMock(
        return_value={"symbol": "AAPL", "price": 150.0, "provider": "YahooFinance"}
    )
    return mock
...
def mock_alpha_vantage_source(monkeypatch):
    ...
    mock.get_quote = AsyncMock(
        return_value={"symbol": "AAPL", "price": 151.0, "provider": "AlphaVantage"}
    )
    return mock
...
@pytest.mark.asyncio
async def test_get_quote_success_primary(
    mock_yahoo_source, mock_alpha_vantage_source, mock_fred_source
):
    """Test that get_quote returns data from the primary provider on success."""
    gateway = DataGateway(
        providers=[mock_yahoo_source, mock_alpha_vantage_source, mock_fred_source]
    )
...
@pytest.mark.asyncio
async def test_get_quote_fallback(
    mock_yahoo_source, mock_alpha_vantage_source, mock_fred_source
):
...
@pytest.mark.asyncio
async def test_get_quote_all_fail(
    mock_yahoo_source, mock_alpha_vantage_source, mock_fred_source
):
